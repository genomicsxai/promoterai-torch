...
mkdir -p examples/track_benchmark/data/track_parity

# Compare full predicted tracks on deterministic random one-hot sequences.
for model in "${models[@]}"; do
    echo time python examples/track_benchmark/compare_tf_torch_tracks_random.py -v \
        --keras_model models/${model} \
        --torch_checkpoint models/${model}.pt \
        --n_sequences 16 \
        --batch_size 1 \
        --tf_device gpu \
        --output_csv examples/track_benchmark/data/track_parity/${model}.random.csv
done | simple_gpu_scheduler --gpus 0,1

# Compare full predicted tracks on selected real promoters.
for model in "${models[@]}"; do
    echo time python examples/track_benchmark/compare_tf_torch_tracks_promoters.py -v \
        --keras_model models/${model} \
        --torch_checkpoint models/${model}.pt \
        ...
        --batch_size 1 \
        --tf_device gpu \
        --output_csv examples/track_benchmark/data/track_parity/${model}.DNAJC9_TERT_SFSWAP.csv \
