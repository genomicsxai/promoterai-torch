...
def test_single_finetune_step_gradient_and_optimizer_equivalence(tmp_path):
    ...
    assert_pass_rate(
        ...
        min_pass_rate=1.0, label="Keras backbone BatchNorm running stats changed despite being frozen",
    )
    assert_pass_rate(
        results["bn_unchanged_pt"], cosine_threshold=1.0, rel_l2_tol=1e-9,
        min_pass_rate=1.0, label="PyTorch backbone BatchNorm running stats changed despite TwinModel.eval()",
    )
